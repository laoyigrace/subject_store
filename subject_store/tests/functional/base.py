...
class Base(testtools.TestCase):
    ...
    def setUp(self):
        ...
        CONF.set_override('stores', [self.driver_name], group='subject_store')
        CONF.set_override('default_store',
                          [self.driver_name],
                          group='subject_store'
                          )
        subject_store.create_stores()
        self.store = subject_store.backend._load_store(CONF, self.driver_name)
...
class BaseFunctionalTests(Base):
    ...
    def test_delete(self):
        ...
        location = subject_store.location.get_location_from_uri(loc)
