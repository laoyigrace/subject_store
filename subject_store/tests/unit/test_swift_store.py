...
def stub_out_swiftclient(stubs, swift_store_auth_version):
    ...
    fixture_headers = {
        ...
        'subject/%s' % FAKE_UUID2: {'x-static-large-object': 'true', },
    }
    fixture_objects = {'subject/%s' % FAKE_UUID: six.BytesIO(b"*" * FIVE_KB),
                       'subject/%s' % FAKE_UUID2: six.BytesIO(b"*" * FIVE_KB), }
    ...
    def fake_put_object(url, token, container, name, contents, **kwargs):
        ...
        if fixture_key not in fixture_headers:
            ...
            if read_len > MAX_SWIFT_OBJECT_SIZE:
                msg = ('Subject size:%d exceeds Swift max:%d' %
...
class SwiftTests(object):
    ...
    def test_get_size(self):
        ...
        """
        uri = "swift://%s:key@auth_address/subject/%s" % (
    ...
    def test_get_with_http_auth(self):
        ...
        loc = location.get_location_from_uri(
            "swift+http://%s:key@auth_address/subject/%s" %
    ...
    def test_get_non_existing(self):
        ...
        """
        loc = location.get_location_from_uri(
            "swift://%s:key@authurl/subject/noexist" % (self.swift_store_user),
            conf=self.conf)
        self.assertRaises(exceptions.NotFound,
                          self.store.get,
                          loc)

    @mock.patch('subject_store._drivers.swift.utils'
    ...
    def test_add(self):
        ...
        loc = "swift+https://tenant%%3Auser1:key@localhost:8080/subject/%s"
    ...
    def test_add_multi_store(self):
        ...
        global SWIFT_PUT_OBJECT_CALLS
        SWIFT_PUT_OBJECT_CALLS = 0
        loc = 'swift+config://store_2/subject/%s'
    ...
    def test_add_auth_url_variations(self):
        ...
        variations = {
            'store_4': 'swift+config://store_4/subject/%s',
            'store_5': 'swift+config://store_5/subject/%s',
            'store_6': 'swift+config://store_6/subject/%s'
        }
    ...
    def test_add_large_object(self):
        ...
        expected_checksum = hashlib.md5(expected_swift_contents).hexdigest()
        expected_image_id = str(uuid.uuid4())
        loc = 'swift+config://ref1/subject/%s'
        expected_location = loc % (expected_image_id)
        image_swift = six.BytesIO(expected_swift_contents)
    ...
    def test_delete(self):
        ...
        uri = "swift://%s:key@authurl/subject/%s" % (
            self.swift_store_user, FAKE_UUID)
        loc = location.get_location_from_uri(uri, conf=self.conf)
    ...
    def test_delete_slo(self, mock_del_obj):
        ...
        uri = "swift://%s:key@authurl/subject/%s" % (self.swift_store_user,
                                                    FAKE_UUID2)
        loc = location.get_location_from_uri(uri, conf=self.conf)
        self.store.delete(loc)
    ...
    def test_delete_with_reference_params(self):
        ...
        uri = "swift+config://ref1/subject/%s" % (FAKE_UUID)
        loc = location.get_location_from_uri(uri, conf=self.conf)
        self.store.delete(loc)
    ...
    def test_delete_with_some_segments_failing(self):
        ...
        loc_uri = "swift+https://%s:key@localhost:8080/subject/%s"
    ...
    def test_read_acl_public(self):
        ...
        store.configure()
        uri = "swift+http://storeurl/subject/%s" % FAKE_UUID
        loc = location.get_location_from_uri(uri, conf=self.conf)
        ctxt = mock.MagicMock()
    ...
    @mock.patch("subject_store._drivers.swift."
    ...
    @mock.patch("subject_store._drivers.swift.store.ks_v3")
    @mock.patch("subject_store._drivers.swift.store.ks_session")
    @mock.patch("subject_store._drivers.swift.store.ks_client")
...
class TestStoreAuthV2(TestStoreAuthV1):
    ...
    def test_v2_with_no_tenant(self):
        uri = "swift://failme:key@auth_address/subject/%s" % (FAKE_UUID)
        loc = location.get_location_from_uri(uri, conf=self.conf)
    ...
    def test_v2_multi_tenant_location(self):
        ...
        uri = "swift://auth_address/subject/%s" % (FAKE_UUID)
        loc = location.get_location_from_uri(uri, conf=self.conf)
...
class TestSingleTenantStoreConnections(base.StoreBaseTest):
    ...
    def setUp(self):
        ...
        self.store.configure()
        specs = {'scheme': 'swift',
                 'auth_or_store_url': 'example.com/v1/',
                 'user': 'tenant:user1',
                 'key': 'key1',
                 'container': 'cont',
                 'obj': 'object'}
    ...
    def test_basic_connection(self):
        ...
        self.assertEqual('https://example.com/v1/', connection.authurl)
    ...
    def test_connection_with_no_trailing_slash(self):
        self.location.auth_or_store_url = 'example.com/v1'
        connection = self.store.get_connection(self.location)
        self.assertEqual('https://example.com/v1/', connection.authurl)
