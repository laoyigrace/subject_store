...
"""
A class that describes the location of an subject in Glance.

In Glance, an subject can either be **stored** in Glance, or it can be
...
describes where exactly an subject is stored.

An subject in Glance has two location properties: the subject URI
and the subject storage URI.

The subject URI is essentially the permalink identifier for the subject.
...
security credential information at all. The Glance subject URI shall
be the host:port of that Glance API server along with /subjects/<IMAGE_ID>.

The Glance storage URI is an internal URI structure that Glance
uses to maintain critical information about how to access the subjects
...
def get_location_from_uri(uri, conf=CONF):
    ...
    Example URIs:
        https://user:pass@example.com:80/subjects/some-id
        http://subjects.oracle.com/123456
        ...
        file:///var/lib/subject/subjects/1
        cinder://volume-id
    """
...
class Location(object):
    ...
    """
    Class describing the location of an subject that Glance knows about
    """

    def __init__(self, store_name, store_location_class, conf,
                 uri=None, subject_id=None, store_specs=None):
        """
        ...
        :param subject_id: The identifier of the subject in whatever storage
                         backend is used.
        :param uri: Optional URI to construct location from
        :param store_specs: Dictionary of information about the location
                            of the subject that is dependent on the backend
                            store
        """
    ...
    def get_store_uri(self):
        """
        Returns the Glance subject URI, which is the host:port of the API server
        along with /subjects/<IMAGE_ID>
        """
