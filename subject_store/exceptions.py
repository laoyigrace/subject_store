...
class NotFound(GlanceStoreException):
    message = _("Subject %(subject)s not found")
...
class Duplicate(GlanceStoreException):
    message = _("Subject %(subject)s already exists")


class StorageFull(GlanceStoreException):
    message = _("There is not enough disk space on the subject storage media.")


class StorageWriteDenied(GlanceStoreException):
    message = _("Permission to write subject storage media denied.")
...
class StoreDeleteNotSupported(GlanceStoreException):
    message = _("Deleting subjects from this store is not supported.")


class StoreGetNotSupported(GlanceStoreException):
    message = _("Getting subjects from this store is not supported.")


class StoreRandomGetNotSupported(StoreGetNotSupported):
    message = _("Getting subjects randomly from this store is not supported. "
                "Offset: %(offset)s, length: %(chunk_size)s")


class StoreAddDisabled(GlanceStoreException):
    message = _("Configuration for store failed. Adding subjects to this "
                "store is disabled.")
...
class RegionAmbiguity(GlanceStoreException):
    message = _("Multiple 'subject' service matches for region %(region)s. This "
                "generally means that a region is required and you have not "
                "supplied one.")


class RemoteServiceUnavailable(GlanceStoreException):
    message = _("Remote server where the subject is present is unavailable.")


class HasSnapshot(GlanceStoreException):
    message = _("The subject cannot be deleted because it has snapshot(s).")


class InUseByStore(GlanceStoreException):
    message = _("The subject cannot be deleted because it is in use through "
