...
from subject_store.i18n import _, _LE, _LW, _LI
...
Possible values:
    * URL template string for cinder endpoint, where ``%%(tenant)s`` is
      ...
      For example: ``http://cinder.openstack.example.org/v1/%%(tenant)s``
...
When the cinder volume is created, deleted, or attached to the subject node to
...
to an unexpected value (e.g. `error``), the subject creation fails.
...
This is useful to hide the subject volumes from users by storing them in a
project/tenant specific to the subject service. It also enables users to share
the subject volume among other projects under the control of subject's ACL.
...
Possible values:
    * A valid authentication service address, for example:
      ``http://openstack.example.org/identity/v1.0``
...
""")),
    cfg.StrOpt('cinder_store_project_name',
               default=None,
               help=_("""
Project name where the subject volume is stored in cinder.
...
""")),
    cfg.StrOpt('rootwrap_config',
               default='/etc/subject/rootwrap.conf',
               help=_("""
Path to the rootwrap configuration file to use for running commands as root.

The cinder store requires root privileges to operate the subject volumes (for
...
class Store(subject_store.driver.Store):
    ...
    def get(self, location, offset=0, chunk_size=None, context=None):
        ...
        :param chunk_size: size to read, or None to get all the subject
        ...
        try:
            ...
            size = int(volume.metadata.get('subject_size',
            ...
            reason = _("Failed to get subject size due to "
                       "volume can not be found: %s") % volume.id
            LOG.error(reason)
            raise exceptions.NotFound(reason)
        except cinder_exception.ClientException as e:
            msg = (_('Failed to get subject volume %(volume_id): %(error)s')
    ...
    def get_size(self, location, context=None):
        ...
        try:
            ...
            return int(volume.metadata.get('subject_size',
                                           volume.size * units.Gi))
        except cinder_exception.NotFound:
            raise exceptions.NotFound(subject=loc.volume_id)
        except Exception:
            LOG.exception(_LE("Failed to get subject size due to "
                              "internal error."))
            return 0
    ...
    def add(self, subject_id, subject_file, subject_size, context=None,
        ...
        bytes_written = 0
        size_gb = int(math.ceil(float(subject_size) / units.Gi))
        if size_gb == 0:
            size_gb = 1
        ...
        LOG.debug('Creating a new volume: subject_size=%d size_gb=%d',
                  subject_size, size_gb)
        if subject_size == 0:
            LOG.info(_LI("Since subject size is zero, we will be doing "
        ...
        try:
            while need_extend:
                with self._open_cinder_volume(client, volume, 'wb') as f:
                    ...
                    while True:
                        buf = subject_file.read(self.WRITE_CHUNKSIZE)
                        if not buf:
                            need_extend = False
                            break
                        checksum.update(buf)
        ...
        if subject_size == 0:
            metadata.update({'subject_size': str(bytes_written)})
    ...
    def delete(self, location, context=None):
        ...
        :location `subject_store.location.Location` object, supplied
                  from subject_store.location.get_location_from_uri()
