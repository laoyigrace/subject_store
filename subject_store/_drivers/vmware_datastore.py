...
from subject_store.i18n import _, _LE
...
""")),
    cfg.StrOpt('vmware_store_subject_dir',
               default=DEFAULT_STORE_IMAGE_DIR,
               help=_("""
The directory where the subject subjects will be stored in the datastore.
...
""")),
    cfg.MultiStrOpt(
        'vmware_datastores',
        help=_("""
The datastores where the subject can be stored.

This configuration option specifies the datastores where the subject can
...
When adding an subject, the datastore with highest weight will be
selected, unless there is not enough free space available in cases
where the subject size is already known. If no weight is given, it is
...
class StoreLocation(location.StoreLocation):
    ...
    def process_specs(self):
        ...
        self.path = os.path.join(DS_URL_PREFIX,
                                 self.specs.get('subject_dir').strip('/'),
                                 self.specs.get('subject_id'))
    ...
    # it during subject add/get. This validation relies on a config param
    # which doesn't make sense to have in the StoreLocation instance.
    # def _is_valid_path(self, path):
    #    sdir = self.conf.subject_store.vmware_store_subject_dir.strip('/')
...
class Store(subject_store.Store):
        ...
        for k, v in sorted(six.iteritems(self.datastores), reverse=True):
            ...
            for ds in v:
                ...
                if ds.freespace > max_fs:
                    max_ds = ds
                    max_fs = ds.freespace
            if max_ds and max_ds.freespace >= subject_size:
                return max_ds
        msg = _LE("No datastore found with enough free space to contain an "
                  "subject of size %d") % subject_size
        LOG.error(msg)
    ...
    def add(self, subject_id, subject_file, subject_size, context=None,
        ...
        :raises: `subject.common.exceptions.Duplicate` if the subject already
                existed
                `subject.common.exceptions.UnexpectedStatus` if the upload
                request returned an unexpected status. The expected responses
                are 201 Created and 200 OK.
        """
        ds = self.select_datastore(subject_size)
        subject_file = _Reader(subject_file, verifier)
        headers = {}
        if subject_size > 0:
            headers.update({'Content-Length': six.text_type(subject_size)})
            data = subject_file
        else:
            data = utils.chunkiter(subject_file, CHUNKSIZE)
        loc = StoreLocation({'scheme': self.scheme,
                             'server_host': self.server_host,
                             'subject_dir': self.store_subject_dir,
                             'datacenter_path': ds.datacenter.path,
                             'datastore_name': ds.name,
                             'subject_id': subject_id}, self.conf)
        ...
        try:
            ...
            with excutils.save_and_reraise_exception():
                LOG.exception(_LE('Failed to upload content of subject '
                                  '%(subject)s'), {'subject': subject_id})

        res = response.raw
        if res.status == requests.codes.conflict:
            raise exceptions.Duplicate(_("Subject file %(subject_id)s already "
                                         "exists!") %
                                       {'subject_id': subject_id})

        if res.status not in (requests.codes.created, requests.codes.ok):
            msg = (_LE('Failed to upload content of subject %(subject)s. '
        ...
        return (loc.get_uri(), subject_file.size,
                subject_file.checksum.hexdigest(), {})
    ...
    def delete(self, location, context=None):
        ...
        try:
            ...
            msg = _('Subject file %s not found') % file_path
            LOG.warning(msg)
            raise exceptions.NotFound(message=msg)
        except Exception:
            with excutils.save_and_reraise_exception():
                LOG.exception(_LE('Failed to delete subject %(subject)s '
                                  'content.') % {'subject': location.subject_id})
        ...
        for i in range(self.api_retry_count + 1):
            ...
            if resp.status >= 400:
                ...
                if resp.status == requests.codes.not_found:
                    reason = _('VMware datastore could not find subject at URI.')
                    LOG.info(reason)
    ...
    def _new_location(self, old_location, url):
        ...
        subject_id = old_location.subject_id
        ...
        return subject_store.location.Location(store_name,
                                               store_class,
                                               self.conf,
                                               uri=vsphere_url,
                                               subject_id=subject_id,
