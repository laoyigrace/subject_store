...
from subject_store._drivers.swift import connection_manager
from subject_store._drivers.swift import utils as sutils
...
can connect to for subject storage. By default, there is no region
set.

When Glance uses Swift as the storage backend to store subjects
...
storing subjects while using Swift backend storage. The default
...
""")),
    cfg.StrOpt('swift_store_container',
               ...
               help=_("""
Name of single container to store subjects/name prefix for multiple containers

When a single container is being used to store subjects, this configuration
option indicates the container within the Glance account to be used for
storing all subjects. When multiple containers are used to store subjects, this
...
set for this configuration option with the first N chars of the subject UUID
as the suffix delimited by an underscore (where N is specified by
``swift_store_multiple_containers_seed``).

Example: if the seed is set to 3 and swift_store_container = ``subject``, then
an subject with UUID ``fdae39a1-bac5-4238-aba4-69bcc726e848`` would be placed in
the container ``subject_fda``. All dashes in the UUID are included when
creating the container name but do not count toward the character limit, so
when N=10 the container name would be ``subject_fdae39a1-ba.``
...
""")),
    cfg.IntOpt('swift_store_large_object_size',
               ...
               help=_("""
The size threshold, in MB, after which Glance will start segmenting subject data.
...
driver will start segmenting subject data into multiple smaller files.
...
""")),
    cfg.IntOpt('swift_store_large_object_chunk_size',
               ...
               help=_("""
The maximum size, in MB, of the segments when subject data is segmented.

When subject data is segmented to upload subjects that are larger than the limit
enforced by the Swift cluster, subject data is broken into segments that are no
...
``swift_store_large_object_chunk_size`` is 1GB, an subject of size 6.2GB will be
...
""")),
    cfg.BoolOpt('swift_store_create_container_on_put', default=False,
                help=_("""
Create container, if it doesn't already exist, when uploading subject.

At the time of uploading an subject, if the corresponding container doesn't
...
""")),
    cfg.BoolOpt('swift_store_multi_tenant', default=False,
                help=_("""
Store subjects in tenant's Swift account.

This enables multi-tenant storage mode which causes Glance subjects to be stored
in tenant specific Swift accounts. If this is disabled, Glance stores all
subjects in its own account. More details multi-tenant store can be found at
...
""")),
    cfg.IntOpt('swift_store_multiple_containers_seed',
               ...
               help=_("""
Seed indicating the number of containers to use for storing subjects.

When using a single-tenant store, subjects can be stored in one or more than one
containers. When set to 0, all subjects will be stored in one single container.
When set to an integer value between 1 and 32, multiple containers will be
used to store subjects. This configuration option will determine how many
...
will be used to store subjects.
...
https://specs.openstack.org/openstack/subject-specs/specs/kilo/swift-store-multiple-containers.html
...
HTTPS Swift requests for subjects at the SSL layer. By default,
compression is enabled.

When using Swift as the backend store for Glance subject storage,
...
performance for subjects which are already in a compressed format,
for example, qcow2.
...
Provide an integer value representing the number of times an subject
download must be retried before erroring out. The default value is
zero (no retry on a failed subject download). When set to a positive
...
Swift. However, some Swift transactions (like uploading subject
...
def swift_retry_iter(resp_iter, length, store, location, manager):
    ...
    while retries <= store.conf.subject_store.swift_store_retry_get_count:
        try:
        ...
        if bytes_read != length:
            if retries == store.conf.subject_store.swift_store_retry_get_count:
                ...
                retries += 1
                subject_conf = store.conf.subject_store
                retry_count = subject_conf.swift_store_retry_get_count
...
class StoreLocation(location.StoreLocation):
    ...
    def get_uri(self, credentials_included=True):
        ...
        if not credentials_included:
            ...
            store = self.conf.subject_store.default_swift_reference
...
def Store(conf):
    ...
    if conf.subject_store.swift_store_multi_tenant:
...
class BaseStore(driver.Store):
    ...
    def configure(self, re_raise_bsc=False):
        subject_conf = self.conf.subject_store
        ...
        self.admin_tenants = subject_conf.swift_store_admin_tenants
        self.region = subject_conf.swift_store_region
        self.service_type = subject_conf.swift_store_service_type
        self.conf_endpoint = subject_conf.swift_store_endpoint
        self.endpoint_type = subject_conf.swift_store_endpoint_type
        self.insecure = subject_conf.swift_store_auth_insecure
        self.ssl_compression = subject_conf.swift_store_ssl_compression
        self.cacert = subject_conf.swift_store_cacert
    ...
    def get(self, location, connection=None,
        ...
        allow_retry = \
            self.conf.subject_store.swift_store_retry_get_count > 0
        with get_manager_for_store(self, location, context,
            ...
            class ResponseIndexable(subject_store.Indexable):
                def another(self):
                    try:
    ...
    @capabilities.check
    def add(self, subject_id, subject_file, subject_size,
            context=None, verifier=None):
        location = self.create_location(subject_id, context=context)
        # initialize a manager with re-auth if subject need to be splitted
        need_chunks = (subject_size == 0) or (
            subject_size >= self.large_object_size)
        with get_manager_for_store(self, location, context,
            ...
            LOG.debug("Adding subject object '%(obj_name)s' "
                      "to Swift" % dict(obj_name=location.obj))
            try:
                if not need_chunks:
                    # Subject size is known, and is less than large_object_size.
                    # Send to Swift with regular PUT.
                    if verifier:
                        checksum = hashlib.md5()
                        reader = ChunkReader(subject_file, checksum,
                                             subject_size, verifier)
                        obj_etag = manager.get_connection().put_object(
                            location.container, location.obj,
                            reader, content_length=subject_size)
                    else:
                        obj_etag = manager.get_connection().put_object(
                            location.container, location.obj,
                            subject_file, content_length=subject_size)
                else:
                    # Write the subject into Swift in chunks.
                    chunk_id = 1
                    if subject_size > 0:
                        total_chunks = str(int(
                            math.ceil(float(subject_size) /
                                      float(self.large_object_chunk_size))))
                    else:
                        # subject_size == 0 is when we don't know the size
                        # of the subject. This can occur with older clients
                        # that don't inspect the payload size.
                        LOG.debug("Cannot determine subject size. Adding as a "
                    ...
                    while True:
                        ...
                        if subject_size == 0:
                            ...
                            left = subject_size - combined_chunks_size
                            if left == 0:
                                break
                            if chunk_size > left:
                                chunk_size = left
                        ...
                        reader = ChunkReader(subject_file, checksum, chunk_size,
                                             verifier)
                    ...
                    # In the case we have been given an unknown subject size,
                    # set the size to the total size of the combined chunks.
                    if subject_size == 0:
                        subject_size = combined_chunks_size
                    ...
                    # the MD5 of the entire subject file contents, so that
                    # users can verify the subject file contents accordingly
                ...
                # subject data. We *really* should consider NOT returning
                # the location attribute from GET /subjects/<ID> and
                ...
                return (location.get_uri(credentials_included=include_creds),
                        subject_size, obj_etag, {})
            except swiftclient.ClientException as e:
                if e.http_status == http_client.CONFLICT:
                    msg = _("Swift already has an subject at this location")
                ...
                msg = (_(u"Failed to add object to Swift.\n"
                       ...
                       % encodeutils.exception_to_unicode(e))
                LOG.error(msg)
                raise subject_store.BackendException(msg)
    ...
    def delete(self, location, connection=None, context=None):
        ...
        try:
            ...
            if e.http_status == http_client.NOT_FOUND:
                msg = _("Swift could not find subject at URI.")
                raise exceptions.NotFound(message=msg)
            else:
                raise
    ...
    def _create_container_if_missing(self, container, connection):
        ...
        try:
            ...
            if e.http_status == http_client.NOT_FOUND:
                if self.conf.subject_store.swift_store_create_container_on_put:
                    try:
    ...
    def create_location(self, subject_id, context=None):
        raise NotImplementedError()
...
class SingleTenantStore(BaseStore):
    ...
    def configure_add(self):
        default_ref = self.conf.subject_store.default_swift_reference
        ...
        if self.auth_address.startswith('http://'):
            ...
            self.scheme = 'swift+https'
        self.container = self.conf.subject_store.swift_store_container
    ...
    def create_location(self, subject_id, context=None):
        container_name = self.get_container_name(subject_id, self.container)
    ...
    def get_container_name(self, subject_id, default_subject_container):
        """
        Returns appropriate container name depending upon value of
        ``swift_store_multiple_containers_seed``. In single-container mode,
        which is a seed value of 0, simply returns default_subject_container.
        In multiple-container mode, returns default_subject_container as the
        prefix plus a suffix determined by the multiple container seed

        examples:
            single-container mode:  'subject'
            multiple-container mode: 'subject_3a1' for subject uuid 3A1xxxxxxx...

        :param subject_id: UUID of subject
        :param default_subject_container: container name from
               ``swift_store_container``
        """
        seed_num_chars = \
            self.conf.subject_store.swift_store_multiple_containers_seed
        if seed_num_chars is None \
            ...
            num_dashes = subject_id[:seed_num_chars].count('-')
            num_chars = seed_num_chars + num_dashes
            name_suffix = subject_id[:num_chars]
            new_container_name = default_subject_container + '_' + name_suffix
            return new_container_name
        else:
            return default_subject_container
...
class MultiTenantStore(BaseStore):
    ...
    def create_location(self, subject_id, context=None):
        ...
        specs = {'scheme': self.scheme,
                 'container': self.container + '_' + str(subject_id),
                 'obj': str(subject_id),
    ...
    def init_client(self, location, context=None):
        ...
        # create client for trustee - subject user specified in swift config
        ...
        # request subject user id - we will use it as trustee user
        ...
        # create client for subject trustee user
...
def get_manager_for_store(store, store_location,
    ...
    :param store_location: StoreLocation object that define subject location
    ...
    if store.__class__ == SingleTenantStore:
        ...
        # with trusts
        if not store.conf.subject_store.swift_store_use_trusts:
            allow_reauth = False
