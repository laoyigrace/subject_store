...
Register the storage backends to use for storing disk subjects
as a comma separated list. The default stores enabled for
storing disk subjects with Glance are ``file`` and ``http``.
...
""")),
    cfg.StrOpt('default_store',
               ...
               help=_("""
The default scheme to use for storing subjects.

Provide a string value representing the default scheme to use for
storing subjects. If not set, Glance uses ``file`` as the default
scheme to store subjects with the ``file`` store.
...
please visit: https://specs.openstack.org/openstack/subject-specs/\
...
http://docs.openstack.org/developer/subject_store/drivers/index.html
...
_STORE_CFG_GROUP = 'subject_store'


def _list_opts():
    driver_opts = []
    mgr = extension.ExtensionManager('subject_store.drivers')
...
class Indexable(object):
    ...
    Store.get() is passed to Store.add() when adding a Copy-From subject to a
...
def _load_store(conf, store_entry, invoke_load=True):
    try:
        ...
        mgr = driver.DriverManager('subject_store.drivers',
                                   store_entry,
                                   invoke_args=[conf],
...
def get_size_from_backend(uri, context=None):
    """Retrieves subject size from backend specified by uri."""
...
def check_location_metadata(val, key=''):
    if isinstance(val, dict):
        ...
        raise exceptions.BackendException(_("The subject metadata key %(key)s "
...
def store_add_to_backend(subject_id, data, size, store, context=None,
                         verifier=None):
    """
    A wrapper around a call to each stores add() method.  This gives subject
    a common place to check the output

    :param subject_id:  The subject add to which data is added
    ...
    """
    (location, size, checksum, metadata) = store.add(subject_id,
                                                     data,
                                                     size,
                                                     context=context,
                                                     verifier=verifier)
...
def add_to_backend(conf, subject_id, data, size, scheme=None, context=None,
                   verifier=None):
    if scheme is None:
        scheme = conf['subject_store']['default_store']
    store = get_store_from_scheme(scheme)
    return store_add_to_backend(subject_id, data, size, store, context,
                                verifier)
