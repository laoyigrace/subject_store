...
function run_tests {
  ...
  if [ $coverage -eq 1 ]; then
    ...
    ${wrapper} coverage html --include='subject_store/*' -d covhtml -i
    ${wrapper} coverage report --include='subject_store/*' -i
  fi
...
function run_pep8 {
  ...
  bash -c "${wrapper} find subject_store -type f -regex '.*\.pot?' -print0|${wrapper} xargs --null -n 1 ${wrapper} msgfmt --check-format -o /dev/null"
}
